...
### GYM-40: Картка «Тренування сьогодні» на вкладці «Сьогодні»
...
Фото → БЖВ (GYM-23/24), картка вправи (GYM-31), профіль (GYM-26), картка
тренування сьогодні (GYM-40) і групові нагадування (Epic 4) додаються
інкрементально, кожен — самодостатній реліз.
...
- **Назва chat-menu-кнопки — вирішено.** `📱 Щоденник` (GYM-35, без змін
  відтоді). Якщо після використання виявиться незрозумілою — це лише
  рядок у `configure_bot_commands` (`src/bot/bot.py`), змінити дешево.
