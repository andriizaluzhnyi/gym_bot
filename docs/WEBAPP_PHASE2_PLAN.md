# ✅ План реалізації (фаза 2): харчування v2, програми тренувань у БД, групові нагадування, меню бота — **виконано**

**Статус: виконано повністю.** Усі таски GYM-21…GYM-43 закриті (кілька —
з відхиленнями від AC, задокументованими в примітках до відповідних
тікетів). Наступний план —
[WEBAPP_PHASE3_PLAN.md](WEBAPP_PHASE3_PLAN.md); туди ж перенесено розділ
«Відкриті питання та ризики» з актуальним статусом кожного пункту.
...
### ✅ GYM-23: Фото → БЖВ — бекенд (OpenAI Vision) — **виконано (з відхиленнями, див. примітку)**
...
**Примітка щодо реалізації:** пакет `openai`, встановлений у це
середовище (`uv add "openai>=1.50"`), розв'язався в `openai==3.9.0` —
значно новіша мажорна версія, ніж очікувалась при написанні AC, з
розширеним списком моделей/параметрів у типах SDK. `chat.completions
.create(...)` з `response_format={"type": "json_schema", ...}` (класичний
Chat Completions API) все ще підтримується в цій версії — саме його й
використано, як просив AC; типи SDK для `messages`/`response_format`
стали строгішими (TypedDict-об'єднання), тож виклик позначено
`# type: ignore[call-overload]` — тіло запиту коректне в рантаймі
(документована форма vision + structured-output запиту), просто не
співпадає статично з дуже вузькими `Literal`-варіантами SDK.

`recognize_food`/`parse_food_estimate` спроєктовані з чіткою межею: якщо
модель відповіла (навіть `is_food: false`) — це `NotFoodError`, окрема
гілка від `InvalidEstimatePayloadError` (модель відповіла чимось
неструктурованим/неповним) — обидві **не** повторюються; повторюється
лише `APITimeoutError`/`InternalServerError` (5xx), рівно раз, як просив
AC. Схема відповіді (`_RESPONSE_JSON_SCHEMA`) **не** містить поля
`calories` взагалі — модель його не питають, бо воно однаково завжди
перераховується з БЖВ (`round(protein*4 + fats*9 + carbs*4)`,
ідентично до `calculateCalories()` в `meal_entry.html`) — AC каже
"перераховується... якщо модель дала неузгоджене число", але
"перевірити узгодженість, і за потреби перерахувати" дає той самий
результат, що й "перерахувати завжди" (коли число вже узгоджене,
перерахунок дає те саме число) — тож не питати модель про calories
взагалі й завжди рахувати самим — простіше і без розбіжності.

Валідація діапазонів — не відхиляє, а **обмежує** (clamp): порція
1-3000 г, кожен БЖВ-компонент 0-500 г. Від'ємне чи неправдоподібно
велике число — це і є той випадок, коли `confidence: "low"` мало б
попереджати користувача, а не привід відкидати всю оцінку.

Мультипарт-парсинг (`_read_meal_photo_field`) рахує розмір по мірі
читання чанками (`read_chunk`), а не за `Content-Length`-заголовком
(його можна підробити або не надіслати) — обрив на першому ж чанку, що
перевищує ліміт, без буферизації решти файлу в пам'яті. `aiohttp`'s
типи для `multipart()`-ітерації допускають вкладений
`MultipartReader | BodyPartReader | None`; звужено явним
`isinstance(part, BodyPartReader)`, що заразом і сама коректна
поведінка (вкладені multipart-частини тут не очікуються).

Ендпоінт-тести (`test_webapp_recognize_meal_photo.py`) — єдиний файл у
проєкті, що ганяє запити через справжній `aiohttp.test_utils.TestClient`/
`TestServer`, а не викликає хендлер напряму: мультипарт-парсинг
(`request.multipart()`) потребує реального потоку запиту, який
`make_mocked_request` не надає. `recognize_food` мокнуто на рівні
`src.webapp.server.recognize_food`; власне звернення до OpenAI
перевірено окремо, повністю мокнутим `AsyncOpenAI`, в
`test_food_recognition.py`.

Перевірено: весь набір тестів (юніт — 31 тест чистої логіки/сервісу в
`test_food_recognition.py`, 15 ендпоінт-тестів, 2 нових у
`test_webapp_user_settings.py`) зелений; наскрізна перевірка реальним
HTTP-запитом через `TestClient` з мокнутим на рівні OpenAI SDK
`AsyncOpenAI` (не мокаючи `recognize_food` — весь шлях: multipart →
`recognize_food` → `parse_food_estimate` → JSON-відповідь).

**SP:** 5

### ✅ GYM-24: Фото → БЖВ — UI (зйомка, прев'ю, підтвердження) — **виконано (не перевірено на реальних клієнтах, див. примітку)**
...
**Примітка щодо реалізації:** обрано `?prefill=<base64url json>`
(перший з двох варіантів AC), не `sessionStorage` — і компактніше для
типового `FoodEstimate` (~100-150 байт у base64, з запасом до будь-яких
практичних лімітів довжини URL), і простіше перевірити наскрізно
(`TestClient` рендерить сторінку з довільним query string; стан
`sessionStorage` довелося б імітувати окремо). Кодування — власний
UTF-8-safe base64url (`TextEncoder` → `btoa` → заміна `+/` на `-_` і
обрізання `=`-падінгу), бо голий `btoa(JSON.stringify(...))` ламається
на не-Latin1 символах, а `meal_name` — українською.

Бейдж не додає окреме поле «порція (г)» у форму — GYM-23's `FoodEstimate
.portion_grams` показується лише як інформаційний текст під бейджем
(«Оцінена порція: ~350 г. Перевірте порцію та БЖУ...»), а не як окремий
input: сама форма `meal_entry.html` ніколи не мала поля порції — БЖУ
там завжди вводяться як абсолютні грами «на порцію», тож порція вже
неявно закодована в цифрах БЖУ. Додавати новий input лише заради фото-
оцінки означало б розширювати форму й для звичайного ручного вводу —
AC цього прямо не просив (технічні підзадачі згадують лише «бейдж»).

Обробка помилок асиметрична, як і просить AC: `422` показує alert і
**лишає користувача на `/nutrition`** (можна одразу перезняти фото);
`502`/`503`, і будь-яка інша неочікувана відповідь/мережева помилка,
показують інший alert і **одразу відкривають порожній `/meal-entry`** —
не блокують ручний ввід через збій AI.

**Не перевірено:** AC явно вимагає перевірки на Telegram iOS/Android/
Desktop, що `input[type=file] capture="environment"` у WebView реально
відкриває камеру/галерею — у цьому середовищі немає доступу до
справжнього клієнта Telegram (лише `esprima`-парсинг синтаксису й
рендер шаблону через `aiohttp.TestClient`, той самий метод, що й у
попередніх UI-тікетах цього спринту). Якщо на якійсь платформі
`input[type=file]` поведеться інакше — фолбек із самого AC (прийом
фото напряму в приватному чаті бота) не в скоупі цього тікета.
...
### ✅ GYM-41: Групування по м'язах, безпечне видалення дня, підказка існуючих вправ — **виконано**
...
**Примітка щодо реалізації:** сортування «найчастіші» для порожнього `q`
не реалізовано — в каталозі немає лічильника використання (AC явно
допускав цей варіант як необов'язковий: «останні/найчастіші»), тож
обрано **найновіші** (`ORDER BY created_at DESC`) — дешево і дає
осмислений результат ("щойно додане" — типовий кандидат для повторного
використання в тій самій сесії редагування програми).

`api_delete_workout_day` з `muscle` мирить дзеркало Sheets окремо:
`GoogleSheetsService.delete_workout_day` вміє видаляти лише **весь**
день, без вибірки по групі, тож звичайний виклик залишив би в Sheets
рядки, яких DB вже позбулась. Замість нього, коли передано `muscle`,
ендпоінт бере список назв вправ цієї групи (через `get_program(day=,
muscle=)` **до** видалення) і мирить кожну окремим викликом
`delete_exercise(user, day, exercise)` — той самий метод, яким уже
мириться одиночне видалення вправи (`api_delete_exercise`). Без
`muscle` — поведінка не змінилась (`delete_workout_day` для всього дня).

Через прибирання `?muscle=` з посилання `nutrition.html` → `/workout`
з'ясувалось, що форма «➕ Додати вправу» в `workout.html` (GYM-30) сама
покладалась на `PARAM_MUSCLE` з URL, щоб не питати групу м'язів —
сторінка вважалась завжди прив'язаною до однієї групи. Тепер, коли
типовий перехід більше не передає `muscle`, ця форма зламалась би
(немає групи для відправки на бекенд). Виправлено додаванням тих самих
чипів вибору групи, що вже є в `nutrition.html`'s версії форми — якщо
`PARAM_MUSCLE` присутній (старе/пряме посилання з конкретною групою),
відповідний чип попередньо вибраний; інакше користувач обирає сам.

`showListView()` рендерить секції-заголовки лише коли `groups.length >
1` — для типового дня з однією групою екран виглядає так само, як до
цього тікета (без зайвого заголовка й дубльованої кнопки видалення).
Кнопка в шапці («Видалити день») перейменовується на «Видалити весь
день» лише в багатогруповому випадку, щоб читалась як ширша дія поряд
із секційними кнопками. Функція групування (`groupExercisesByMuscleGroup`)
винесена як окрема чиста функція (не інлайн у рендер-циклі) — зберігає
оригінальний плаский індекс кожної вправи (`idx` в `exercises[]`), бо
всі наявні обробники (deleteExercise, showDetailView, activeExerciseIndex)
адресують вправи саме за ним, а не за позицією всередині секції.

**Відоме обмеження:** AC просив «юніт-тест чистої функції групування» —
у цьому середовищі немає JS-раннера (той самий, задокументований у
GYM-22/25/26/30/31/37, обмежений лише `esprima`-парсингом синтаксису, без
виконання). Функція навмисно написана як чиста й ізольована (без
DOM/глобальних читань) для майбутнього юніт-тесту, якщо/коли з'явиться
спосіб його запустити в цьому пісочниці; Python-сторона (репозиторій,
ендпоінт) — повністю покрита реальними тестами.

Пре-реліз-знахідка (не пов'язана з GYM-41, але заблокувала чистий
базовий прогін): `tests/test_workout_program_repository.py`'s фікстура
`_create_tables` робила лише `create_all` без `drop_all` — єдиний файл у
проєкті з таким відхиленням від конвенції. Спільний SQLite-файл
(`tests/conftest.py`) переживає окремі запуски `pytest`, тож застарілий
рядок `exercises` (унікальний по `normalized_name`) з попереднього
прогону змушував `get_or_create_by_name` повертати його замість
створення нового — саме це ламало
`TestExerciseRepositoryGetOrCreateByName` час від часу. Виправлено
додаванням `drop_all`, як і в кожному іншому тестовому файлі.

Перевірено: весь набір тестів (юніт — 62 нових/оновлених у
`test_workout_program_repository.py`/`test_webapp_workout_program.py`/
`test_webapp_add_program_exercise.py`) зелений; наскрізна перевірка
реальним HTTP-запитом через `aiohttp.TestClient` (додати вправу в дві
групи одного дня → `GET` без `muscle` показує обидві → `DELETE` з
`muscle` стирає лише одну → `GET /api/exercises` без `q` повертає
нещодавні записи каталогу); JS обох шаблонів перевірено `esprima`-
парсером.
...
- ✅ **GYM-38 — Інтеграційні тести нових ендпоінтів — виконано.** Новий
  `tests/test_webapp_phase2_integration.py`, за зразком
  `tests/test_webapp_statistics_integration.py` (реальний `TestClient`/
  `TestServer`, не виклик хендлера напряму) — 38 тестів: `401` (відсутній/
  «сміттєвий» `Authorization`) на кожному новому `GET`/`POST`/`DELETE`
  `/api/*` цього плану (`/api/statistics/*` не повторюється — це вже
  GYM-19), `403` за чужий `user` для не-адміна на всіх чотирьох
  `/api/workout/program*`-ендпоінтах, що його приймають (плюс один `200`
  для адміна — підтвердити, що гейт саме на «не-адмін», а не «завжди»),
  `404`/`405` на маршрутизацію й `{id}`-параметр `/api/exercises/{id}`,
  multipart на `/api/nutrition/meal/photo` через справжній
  `FormData`-запит (мокнутий лише `recognize_food` — власне звернення до
  OpenAI вже повністю покрите в `test_food_recognition.py`, а
  валідація/маппінг помилок ендпоінта — в `test_webapp_recognize_meal_
  photo.py` з GYM-23; тут — лише підтвердження, що маршрут і мультипарт-
  парсинг справді працюють під `create_webapp()`), і наскрізний сценарій
  з AC («додати вправу → побачити в списку → залогувати сет →
  побачити в `/api/statistics/exercises`») — останній крок особливо
  показовий: `/api/statistics/exercises` (GYM-5a, з фази 1) читає з
  **залогованих сетів**, а не з каталогу програми, тож цей тест реально
  зв'язує GYM-30 (WebApp-редагування програми) з давнішим
  статистичним конвеєром, а не просто повторює вже існуючі юніт-тести
  двох ендпоінтів по черзі. Ендпоінти сесії/логування тренування
  (`/api/workout/session/*`, `/api/workout/log`, `/api/workout/last-log`,
  `/api/workout/rest-timer`) — це GYM-2c, з'явились до цього плану, тож
  не входять у список «нових» для окремих `401`-перевірок; `/api/workout/
  log` все одно використано як крок наскрізного сценарію, як і просить
  AC. **SP: 3**
- ✅ **GYM-39 — Документація — виконано.** Оновлено `README.md`:
  «Функціонал» (фото → БЖВ, DB-програми тренувань, групові нагадування),
  «Команди бота» (виправлено застарілий текст кнопки меню «📱 Щоденник»
  → «📱» після пост-релізної зміни в GYM-35; додано `/start <section>`
  deep-link і нову підсекцію «У групі (нагадування)» з `/reminders`),
  «База даних» (нові таблиці `exercises`, `workout_program_exercises`,
  `group_chats` — `daily_nutrition`/`profiles` уже були задокументовані
  раніше), «Google Sheets структура» (тепер явно назване опційним
  дзеркалом програм **і** логів, а не лише логів; повний перелік
  аркушів, яких бракувало — «Програми», «Програми (Візуалізація)»,
  «Логи (<user>)»; крок імпорту GYM-29), новий розділ «OpenAI» (як
  отримати ключ, орієнтовна вартість, явно — фото ніде не зберігається).
  `.env.example` вже містив `OPENAI_API_KEY`/`OPENAI_MODEL` з GYM-23 —
  без змін.

  Дорогою виправлено дві реальні розбіжності в наявному README, не
  згадані в AC, але прямо в зоні цього тікета: (1) інлайн-приклад `.env`
  у кроці 4 використовував `ADMIN_USER_IDS=123456789,987654321`
  (множина, через кому) — а `src/config.py` читає лише одиничний
  `admin_user_id: int`, тож цей рядок ніколи не спрацював би;
  замінено на `ADMIN_USER_ID=123456789`, як у справжньому
  `.env.example`. (2) Розділ «Документація» і крок 6 посилались на
  `ALEMBIC_QUICKSTART.md`/`MIGRATION_USERS_PROFILES.md`/
  `MIGRATION_TO_UUID.md` в корені репозиторію — файли фактично лежать у
  `docs/`, тож усі три посилання вели в нікуди; виправлено на
  `docs/...`, і туди ж додано лінки на обидва плани фаз
  (`WEBAPP_STATISTICS_PLAN.md`, `WEBAPP_PHASE2_PLAN.md`, яких у списку
  документації не було зовсім). «Структура проекту» також оновлено —
  нові файли `group_reminders.py` (bot + service),
  `food_recognition.py`, `exercise_names.py`,
  `workout_program_parsing.py`, `import_workout_programs.py`, і
  переставлено `docs/`-піддерево на місце, де воно реально лежить.

  Перевірено: повний прогін тестів/flake8/mypy — зміни лише в
  `README.md`, код не займали. **SP: 1**
- ✅ **GYM-42 — Профіль: двосторонній автоперерахунок калорій ↔ БЖВ —
  виконано.** `profile.html`: новий блок біля «Взяти розраховані»
  (GYM-26) додає `MACRO_SPLIT` (30/25/45 %) як спільну константу для обох
  напрямків і `applyCaloriesToMacros()`, яку тепер використовує і сам
  «Взяти розраховані» (раніше дублював той самий розподіл інлайн). Живий
  перерахунок — окремі слухачі `input` на `caloriesInput` (розкладає в
  `proteinInput`/`fatsInput`/`carbsInput` за 30/25/45 %) і на кожному з
  трьох полів БЖВ (сумує назад у `caloriesInput` за `protein*4 + fats*9 +
  carbs*4`, як і бекенд у `update_nutrition_settings`, GYM-23) — додані
  **поверх** наявного загального `.form-input`-слухача
  (`markChanged`/`validateField`/`calculateMetrics`), а не замість нього,
  тож валідація й трек змін для решти полів (вік/зріст/вага/вода) не
  зачеплені. Рекурсії немає: перерахунок ставить значення інших полів
  через `.value`, що не генерує власної події `input`, тож поле, яке
  користувач саме редагує, ніколи не перезаписується самим собою.
  Порожнє/нульове поле-джерело (наприклад, стерли «Калорії») просто не
  запускає перерахунок, лишаючи решту полів як є — рекурсивного
  перерахунку «в нікуди» AC і не вимагав. Округлення — той самий
  `Math.round`, що й у бекенді. Збереження — без змін, той самий
  `tg.MainButton`-флоу.

  Юніт-тестів JS у репозиторії немає (немає JS test-раннера — лише
  Python), тож новий `tests/test_webapp_profile_page.py` (за зразком
  `tests/test_webapp_nutrition_page.py`) — smoke-тест на віддану
  `/profile`-сторінку: підтверджує, що `MACRO_SPLIT`,
  `applyCaloriesToMacros` і обидва напрямки слухачів (`caloriesInput` →
  БЖВ, БЖВ → `caloriesInput`) справді присутні в HTML/inline-JS, що
  віддається клієнту. Сама математика розподілу (4/9/4, 30/25/45 %) уже
  покрита на бекенді в `tests/test_webapp_user_settings.py`.
  **SP: 3**
- ✅ **GYM-43 — Ліміт 5 фото-розпізнавань на добу — виконано.** Нова
  таблиця `photo_recognition_log` (один рядок на спробу розпізнавання —
  за зразком `daily_nutrition`, не лічильник-поле, щоб перевикористати
  вже готовий `period_bounds_utc("day", ...)`-запит на «локальну добу»,
  GYM-21) і `PhotoRecognitionLogRepository.count_for_local_day`/`create`
  (`src/database/models.py`, `repository.py`; міграція
  `05c4b1801db5_add_photo_recognition_log`). `api_recognize_meal_photo`
  (`src/webapp/server.py`) тепер: `503` (фіча вимкнена) → перевірка
  ліміту (`429 {'error': 'daily_photo_limit_exceeded', 'limit', 'reset_at'}`,
  раніше за читання multipart, щоб не витрачати час на парсинг, коли
  ліміт і так вичерпано) → валідація файлу (`400`/`413`, як і раніше, лічильник
  не займають) → лічильник **записується одразу перед викликом
  `recognize_food`** (і комітиться), а не після — тож і `422 not_food`, і
  `502 recognition_failed` рахуються нарівні з `200`, бо OpenAI-запит уже
  стався. Ліміт — `settings.openai_daily_photo_limit`
  (`OPENAI_DAILY_PHOTO_LIMIT`, за замовчуванням `5` — спершу поставили
  `3`, за проханням користувача підняли одразу після реалізації), не
  хардкод.
  Фронтенд (`nutrition.html`, фото-флоу GYM-23/24): окрема гілка для
  `429` з повідомленням про вичерпаний денний ліміт (замість generic «не
  вдалося розпізнати», яка лишилась для `502`/`503`) — і так само
  переводить на `/meal-entry` для ручного вводу.

  Тести — `tests/test_webapp_recognize_meal_photo.py` (нові кейси в
  `TestDailyLimit`, читають ліміт із `settings.openai_daily_photo_limit`,
  а не хардкодять число): рівно `LIMIT` успішних викликів проходять,
  наступний — `429`; ліміт окремий на кожного користувача;
  `422`/`502` рахуються, `400`/`413`/`503`
  (до виклику `recognize_food`) — ні; лічильник «скидається» на новій
  локальній добі (перевірено зсувом `created_at` уже залогованих спроб
  у минуле, а не монкіпатчем часу — обробник сам бере поточний `now`).
  Плюс smoke-тест у `test_webapp_nutrition_page.py` на наявність гілки
  `429` у відданому JS. README/`.env.example` — новий
  `OPENAI_DAILY_PHOTO_LIMIT` поруч з `OPENAI_API_KEY`/`OPENAI_MODEL`,
  окремий підрозділ «Денний ліміт» під «OpenAI». Міграцію на живу
  Postgres-БД (production docker-compose) не застосовував — лише
  перевірив ланцюжок ревізій (`alembic heads` → одна голова).
  **SP: 3**
...
8. **Sprint 8:** GYM-42, GYM-43 — автоперерахунок калорій ↔ БЖВ у профілі,
   денний ліміт фото-розпізнавання

## Відкриті питання та ризики

Розділ перенесено в
[WEBAPP_PHASE3_PLAN.md → «Відкриті питання та ризики» → «Перенесено з фаз 1–2»](WEBAPP_PHASE3_PLAN.md#відкриті-питання-та-ризики),
де закриті пункти (`is_admin()`, Sheets як джерело істини, ліміти OpenAI,
назва chat-menu-кнопки, кнопки розкладу, порядок релізу GYM-28 → GYM-29)
позначено одним рядком, а досі відкриті (username як ключ Sheets,
точність і приватність фото, `input[type=file]` у WebView, наповнення
каталогу вправ, дублікати назв, заміри/фото прогресу, forum-теми, одна
таймзона, відсутність JS-раннера) — перенесено з поточним статусом.
Нижче — стан на момент закриття фази 2, без змін.
...
- **Вартість і ліміти OpenAI.** Орієнтовно $0.002–0.01 за фото при
  ...
  **Статус:** закривається в GYM-43 — ліміт 5 фото-розпізнавань на добу
  на користувача.
