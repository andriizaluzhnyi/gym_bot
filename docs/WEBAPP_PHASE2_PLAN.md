...
- ✅ **GYM-42 — Профіль: двосторонній автоперерахунок калорій ↔ БЖВ —
  ...
  **SP: 3**
- **GYM-43 — Ліміт 3 фото-розпізнавання на добу.** Ризик «Вартість і
  ліміти OpenAI» (розділ «Відкриті питання») згадував `daily_photo_limit`
  як окремий таск на випадок ширшого кола користувачів — цей таск його
  реалізує: `POST /api/nutrition/meal/photo` (`api_recognize_meal_photo`,
  `src/webapp/server.py`) обмежується **3 успішними викликами
  розпізнавання на користувача на добу** (локальна доба користувача, як і
  лічильники GYM-21, а не UTC). **AC:**
  - Лічильник по `user_id` за поточну добу; окремий рядок/поле в БД (за
    зразком добових лічильників `daily_nutrition`) або окрема таблиця
    `photo_recognition_log` — обрати за наявною схемою лічильників,
    без нового зовнішнього стору.
  - Ліміт рахує лише виклики, що дійшли до `recognize_food` (реальний
    OpenAI-запит); `400`/`413`/`503` до виклику (невалідний файл,
    вимкнена фіча) лічильник не витрачають. `422 not_food` — питання для
    рішення в цьому тікеті: рахувати як спробу (бо OpenAI-запит все одно
    пішов і кошти списані) чи ні — раджу рахувати, бо ризик у README
    саме про вартість запитів, а не про «корисні» розпізнавання.
  - Понад ліміт — `429` з тілом на кшталт `{'error': 'daily_photo_limit_exceeded', 'limit': 3, 'reset_at': ...}`;
    фронтенд (`meal-entry`/фото-флоу GYM-24) показує зрозуміле повідомлення
    замість generic «не вдалося розпізнати».
  - Ліміт — константа в конфізі (`OPENAI_DAILY_PHOTO_LIMIT` чи подібне,
    поруч із `OPENAI_API_KEY`/`OPENAI_MODEL`), не хардкод у хендлері, щоб
    міняти без деплою коду.
  - Тести: 3 успішних виклики проходять, 4-й — `429`; лічильник скидається
    на новій локальній добі; не рахує неуспішні (400/413/503) виклики.
  - README/`.env.example` (перетинається з GYM-39): згадати ліміт і нову
    змінну середовища.
  **SP: 3**
...
- **Вартість і ліміти OpenAI.** Орієнтовно $0.002–0.01 за фото при
  ...
  **Статус:** закривається в GYM-43 — ліміт 3 фото-розпізнавання на добу
  на користувача.
