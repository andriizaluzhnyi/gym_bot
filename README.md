...
#### Формат «Підходи/Повторення» (`sets_reps`)

Одна вправа в програмі — одне поле `sets_reps`, і в боті (FSM створення
програми), і в Mini App (`/nutrition?tab=workout`, `/workout`):

- Один блок — `3/10` (3 підходи по 10 повторень); також приймаються
  роздільники `|` і `x`/`х` (кирилична) — `3|10`, `3x10`, `3х10`.
- Кілька блоків підряд, через кому — `2/12, 4/6` (2 розминкові підходи по
  12, потім 4 робочі по 6). Довільна кількість пробілів навколо
  роздільника й коми теж приймається (`2/12,4x6` — так само валідно).
- Голе число (`5`) — лише підходи, без запланованих повторень; має сенс
  тільки як єдиний блок, не в списку через кому.

Значення завжди зберігається в канонічному вигляді (`/`, кома + пробіл
між блоками) незалежно від того, які роздільники ввів користувач —
`normalize_sets_reps`/`parse_sets_reps`
(`src/services/workout_program_parsing.py`), спільні для бота й
`POST`/`PATCH /api/workout/program/exercise`. Ліміт — 50 символів
(колонка `workout_program_exercises.sets_reps`); довший ввід відхиляється
з `400` до запису в БД. Вільний текст без роздільника (наприклад,
«до відмови») бот приймає й зберігає як є — валідація формату є лише на
боці Mini App.
