...
- **GYM-42 — Профіль: двосторонній автоперерахунок калорій ↔ БЖВ.** Зараз
  `profile.html` синхронізує калорії й БЖВ лише в один бік і лише за
  кнопкою «📐 Взяти розраховані» (з TDEE, фіксований розподіл 30/25/45 %,
  див. GYM-26). Потрібно, щоб зміна будь-якого з чотирьох полів («Норма
  калорій», «Норма білків», «Норма жирів», «Норма вуглеводів») одразу
  перераховувала решту: правка `caloriesInput` розкладає нове число в
  `proteinInput`/`fatsInput`/`carbsInput` за тим самим співвідношенням
  30/25/45 % (4/9/4 ккал/г), а правка будь-якого з полів БЖВ — навпаки,
  перераховує `caloriesInput` як суму (`protein*4 + fats*9 + carbs*4`), як
  уже робить бекенд для `daily_calories` у
  `update_nutrition_settings`/фото-оцінці (GYM-23, 4/9/4). **AC:**
  - Слухач на `input`/`change` для всіх чотирьох полів (замінити чи
    доповнити нинішній `applyCalculated`); ручне редагування одного поля
    не запускає рекурсивний перерахунок самого себе.
  - Розподіл БЖВ при перерахунку «від калорій» — той самий фіксований
    30/25/45 %, що й «Взяти розраховані»; при перерахунку «від БЖВ» —
    проста сума, без зміни введених користувачем грамів.
  - Округлення узгоджене з бекендом (`round()`), щоб після збереження
    `GET /api/user/settings` не «стрибав» відображеним числом.
  - Перерахунок — лише в інпутах (як і «Взяти розраховані»); збереження
    так само через `tg.MainButton`, без нового round-trip на кожне
    натискання клавіші.
  - Тест: зміна `caloriesInput` оновлює три інших поля; зміна одного з
    БЖВ-полів оновлює `caloriesInput` і не чіпає інші два БЖВ-поля.
  **SP: 3**
