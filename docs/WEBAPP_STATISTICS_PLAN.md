...
### ✅ GYM-12: Розрахунок стріку тренувань — **виконано**
...
**Примітка щодо реалізації:** `src/services/streak.py`, за зразком
`src/services/personal_records.py` (GYM-7) — чиста функція без БД, тижні
ідентифікуються через `date.isocalendar()`/`date.fromisocalendar()`
(ISO-тиждень, понеділок-неділя). `current_streak` рахує назад від тижня
`today`: якщо в ньому ще нема сесії — це не обриває лічилку, а просто
пропускається (рахунок продовжується з попереднього тижня); обриває лише
перший *завершений* тиждень без жодної сесії. `longest_streak` — найдовша
послідовність тижнів із сесією серед усіх `session_dates` (завжди
`>= current_streak`, бо той сам є такою послідовністю). Хендлер
`api_get_streak` — той самий `get_sessions_by_period(owner_id)` без меж,
що й GYM-8/9, дати сесій — через `to_local_date` (`settings.timezone`,
конвенція GYM-4/5a/6). UI-картка/бейдж — поза скоупом цього тікета.
