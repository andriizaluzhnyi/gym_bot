# Gym Bot 🏋️122

Telegram бот для управління записами на тренування з синхронізацією Google Calendar та Google Sheets.

## Функціонал

- 📅 **Розклад тренувань** — перегляд доступних тренувань
- 📝 **Запис на тренування** — онлайн запис з перевіркою вільних місць
- 🔔 **Нагадування** — автоматичні нагадування за 24 години та 2 години
- 📊 **Google Calendar** — синхронізація тренувань з календарем
- 📋 **Google Sheets** — ведення таблиці записів та відвідувань
- 👨‍💼 **Адмін-панель** — управління тренуваннями для тренера

## Встановлення

### 1. Клонування репозиторію

```bash
git clone <repository-url>
cd gym_bot
```

### 2. Створення віртуального середовища

```bash
python -m venv venv
source venv/bin/activate  # Linux/Mac
# або
venv\Scripts\activate  # Windows
```

### 3. Встановлення залежностей

```bash
pip install -e .
```

### 4. Налаштування конфігурації

Скопіюйте `.env.example` в `.env` та заповніть значення:

```bash
cp .env.example .env
```

Відредагуйте `.env`:

```env
# Telegram Bot
TELEGRAM_BOT_TOKEN=your_bot_token_here
ADMIN_USER_IDS=123456789

# Google API
GOOGLE_CREDENTIALS_FILE=credentials.json
GOOGLE_CALENDAR_ID=your_calendar@group.calendar.google.com
GOOGLE_SPREADSHEET_ID=your_spreadsheet_id
```

### 5. Налаштування Google API

#### Google Calendar & Sheets API:

1. Перейдіть на [Google Cloud Console](https://console.cloud.google.com/)
2. Створіть новий проект або оберіть існуючий
3. Увімкніть Google Calendar API та Google Sheets API
4. Створіть Service Account:
   - IAM & Admin → Service Accounts → Create
   - Завантажте JSON ключ як `credentials.json`
5. Надайте доступ Service Account до вашого календаря та таблиці

### 6. Запуск бота

```bash
python -m src.main
```

## Docker

### Запуск через Docker Compose

```bash
# Створіть .env файл
cp .env.example .env
# Відредагуйте .env

# Запустіть
docker-compose up -d

# Перегляд логів
docker-compose logs -f bot
```

## Команди бота

### Для користувачів:
- `/start` — Почати роботу з ботом
- `/help` — Допомога
- `/schedule` — Розклад тренувань
- `/my` — Мої записи
- `/profile` — Мій профіль

### Для адмінів:
- `/admin` — Адмін-панель
- `➕ Додати тренування` — Створити нове тренування
- `📊 Статистика` — Переглянути статистику

## Структура проекту

```
gym_bot/
├── src/
│   ├── bot/
│   │   ├── handlers/
│   │   │   ├── start.py      # Команди /start, /help
│   │   │   ├── schedule.py   # Розклад тренувань
│   │   │   ├── booking.py    # Запис/скасування
│   │   │   └── admin.py      # Адмін функції
│   │   ├── keyboards.py      # Клавіатури
│   │   └── bot.py           # Головний модуль бота
│   ├── services/
│   │   ├── google_calendar.py
│   │   ├── google_sheets.py
│   │   └── notifications.py
│   ├── database/
│   │   ├── models.py        # SQLAlchemy моделі
│   │   ├── repository.py    # Репозиторії
│   │   └── session.py       # Сесії БД
│   ├── config.py            # Конфігурація
│   └── main.py              # Точка входу
├── tests/
├── .env.example
├── docker-compose.yml
├── Dockerfile
└── pyproject.toml
```

## База даних

Бот використовує SQLite за замовчуванням. Структура:

- **users** — користувачі (telegram_id, ім'я, телефон)
- **trainings** — тренування (назва, дата, час, ліміт місць)
- **bookings** — записи (користувач ↔ тренування)

## Google Sheets структура

Бот автоматично створює три листи:

1. **Тренування** — список всіх тренувань
2. **Записи** — записи користувачів
3. **Відвідування** — журнал відвідувань

## Ліцензія

MIT
